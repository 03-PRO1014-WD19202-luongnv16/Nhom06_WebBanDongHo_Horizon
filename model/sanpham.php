<?php
function insert_sanpham($tensp,$giasp,$soluong,$anhsp,$mota,$id_dm,$luotxem,$date){
    $sql="insert into sanpham(tensp,giasp,'soluong',anhsp,mota,id_dm,luotxem,date) values('$tensp','$giasp','$soluong','$anhsp','$mota','$id_dm','$luotxem','$date')";
    pdo_execute($sql);
}
...
function update_sanpham($id_sp,$tensp, $anhsp, $giasp,$soluong, $mota, $id_dm, $luotxem,$date){  
    if($anhsp !== ""){  
        $sql = "update sanpham set tensp='$tensp', anhsp='$anhsp', giasp='$giasp',soluong='$soluong', mota='$mota', id_dm='$id_dm', luotxem='$luotxem', date='$date' where id_sp=$id_sp";  
    } else {  
        $sql = "update sanpham set tensp='$tensp', anhsp='$anhsp', giasp='$giasp',soluong='$soluong' mota='$mota', id_dm='$id_dm', luotxem='$luotxem',date='$date' where id_sp=$id_sp";  
    }  
    pdo_execute($sql);  
